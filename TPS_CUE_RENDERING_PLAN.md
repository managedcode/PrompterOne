...
| `[emphasis]`, `*`, `**` | Distinct editorial treatments: `[emphasis]` uses stronger word shape and weight without connector underlines, markdown bold uses heavier word shape, markdown italic uses a clearly slanted italic word shape. | Monaco markdown/TPS decorations. | No timing change. | Emphasis attribute/style tests plus no overlap and no stray underline strokes. |
| `[highlight]` | Shaped translucent background behind the cue scope, not a color-only cue or broken half-height fill. | Token + hover. | No timing change. | Highlight remains visible on dark background without stacked word fills. |
