...
- TPS visual cue screenshot work must include a comparison/audit pass that evaluates whether related cues are actually distinguishable in the generated images, including qualitative inspection and objective probes where useful; do not stop at producing screenshot files without comparing the result.
...
- TPS pronunciation and phonetic reader guides are the main reading target in the teleprompter reader: when a guide exists, it must be large enough to read as primary rehearsal text, while the original spelling remains only compact secondary context or an accessible attribute.
...
- TPS articulation visuals must follow music-reading metaphors: `legato` should read like one curved slur/ligature mark spanning the entire cue word or grouped phrase, not a wavy spelling-style underline or separate per-word marks; `staccato` should read as dots.
- TPS legato and other curved reader marks must render as smooth vector-quality strokes at README screenshot scale; do not use CSS gradients or raster tricks that create jagged, stair-stepped, or pixelated curves.
- TPS crescendo, stress, legato, pronunciation, phonetic, emphasis, and other scope-based cue treatments must cover the full cue word or phrase scope when the tag applies to a whole word or phrase; partial-letter or single-glyph decoration is a regression unless the TPS semantics explicitly target only that glyph.
...
- TPS cue matrix reviews must explicitly compare similar treatments side by side and reject identical-looking cue families; screenshots are not acceptable until a reader can tell the intended semantics apart at normal README scale.
...
- Browser-test injected forced diagnostics failures must not create real Sentry issues; keep those scenarios useful for local assertions while filtering or tagging them before outbound telemetry delivery.
...
- Large product-design or architecture planning tasks should use available assistant CLIs such as Claude and Copilot as independent second opinions when the user asks for multi-model research; capture their findings in the root-level plan or task artifact before implementation starts.
...
- All AI provider connections must stay on the Microsoft Agent Framework composition path used by PrompterOne agents and `ChatClientAgent`; do not bypass it with direct provider-specific SDK clients for production graph, assistant, or runtime AI behavior, because Agent Framework is the single configuration, provider selection, and orchestration boundary for every AI integration. Provider SDK objects may exist only as private adapters that feed the Agent Framework path; production calls must not execute through those SDK clients directly.
- AI provider configuration must stay minimal and user-owned: do not ship hardcoded model/deployment presets, token limits, context-size fields, model-type fields, or hidden defaults in Settings or appsettings. A provider entry should capture only the provider kind, editable endpoint/base URL when needed, API key when needed, and user-added model or deployment names, with exactly one active provider at runtime unless a later task introduces explicit routing.
...
- Agent/MCP tools must stay organized by product domain instead of growing as one monolithic catalog: keep editor/script, shell/navigation, settings, media devices, streaming/Go Live, graph, and agent-orchestration tools in domain-specific files or folders with clear ownership.
- Agent/MCP tool names must come from named constants and stay token-efficient: do not use repeated raw string literals for tool names, and do not prefix tool names with the product name when a shorter domain-scoped name is clear.
...
- AI assistant responses must use structured output instead of free-form parsing: separate the visible chat reply from machine-readable document edit operations, and express document changes as exact range-based insert, replace, or delete operations that can be applied independently of the chat text.
...
- Script graph build actions must use the configured LLM extractor when an AI provider is available; tokenizer/vector similarity is only an explicit lower-fidelity fallback button and must not silently replace LLM graph extraction.
