...
public sealed class LibraryThemeFlowTests(StandaloneAppFixture fixture) : AppUiTestBase(fixture), IClassFixture<StandaloneAppFixture>
{
    ...
    public Task LibraryScreen_LightTheme_UsesReadableSidebarCardsAndCreateTiles() =>
        RunPageAsync(async page =>
        {
            ...
            Assert.True(
                HasMinimumChannels(sidebarBackground, BrowserTestConstants.LibraryFlow.MinimumLightSidebarSurfaceChannel),
                $"Expected the light-theme library sidebar surface to stay light, but got rgba({sidebarBackground.R:0.##}, {sidebarBackground.G:0.##}, {sidebarBackground.B:0.##}, {sidebarBackground.A:0.##}).");
            Assert.True(
                HasMinimumChannels(searchBackground, BrowserTestConstants.LibraryFlow.MinimumLightSearchSurfaceChannel),
                $"Expected the light-theme library search surface to stay light, but got rgba({searchBackground.R:0.##}, {searchBackground.G:0.##}, {searchBackground.B:0.##}, {searchBackground.A:0.##}).");
            Assert.True(
                HasMinimumChannels(cardBackground, BrowserTestConstants.LibraryFlow.MinimumLightCardSurfaceChannel),
                $"Expected the light-theme library card surface to stay light, but got rgba({cardBackground.R:0.##}, {cardBackground.G:0.##}, {cardBackground.B:0.##}, {cardBackground.A:0.##}).");
            Assert.True(
                HasMinimumChannels(createScriptBackground, BrowserTestConstants.LibraryFlow.MinimumLightCreateTileSurfaceChannel),
                $"Expected the light-theme New Script tile surface to stay light, but got rgba({createScriptBackground.R:0.##}, {createScriptBackground.G:0.##}, {createScriptBackground.B:0.##}, {createScriptBackground.A:0.##}).");
            Assert.True(
                HasMinimumChannels(createFolderBackground, BrowserTestConstants.LibraryFlow.MinimumLightCreateTileSurfaceChannel),
                $"Expected the light-theme New Folder tile surface to stay light, but got rgba({createFolderBackground.R:0.##}, {createFolderBackground.G:0.##}, {createFolderBackground.B:0.##}, {createFolderBackground.A:0.##}).");
            Assert.True(
                HasMaximumChannels(createButtonColor, BrowserTestConstants.LibraryFlow.MaximumReadableTextChannel),
                $"Expected the light-theme header create button text to stay readable, but got rgba({createButtonColor.R:0.##}, {createButtonColor.G:0.##}, {createButtonColor.B:0.##}, {createButtonColor.A:0.##}).");
            Assert.True(
                HasMaximumChannels(folderAllColor, BrowserTestConstants.LibraryFlow.MaximumReadableSecondaryTextChannel),
                $"Expected the light-theme library sidebar text to stay readable, but got rgba({folderAllColor.R:0.##}, {folderAllColor.G:0.##}, {folderAllColor.B:0.##}, {folderAllColor.A:0.##}).");
