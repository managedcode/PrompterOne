    ...
    string requestedBaseAddress) : IAsyncDisposable
{
    ...
    private WebApplication BuildApplication()
    {
        ...
        {
            context.Response.Headers[UiTestHostConstants.BlazorEnvironmentHeaderName] = UiTestHostConstants.ProductionEnvironmentName;
            return next();
        });
    ...
    private async Task ServeAppAssetAsync(HttpContext context, FileExtensionContentTypeProvider provider)
    {
        ...
        if (!string.IsNullOrWhiteSpace(requestPath) && Path.HasExtension(requestPath))
        {
            ...
            if (TryGetTelemetryTestAssetOverridePath(assetPath, out var overridePath))
            {
                await SendFileIfPresentAsync(context, overridePath, provider);
                return;
            }
    ...
    private bool TryGetTelemetryTestAssetOverridePath(string assetPath, out string overridePath)
    {
        overridePath = string.Empty;

        if (string.Equals(assetPath, UiTestHostConstants.AppSettingsDevelopmentFileName, StringComparison.Ordinal))
        {
            overridePath = Path.Combine(_appWwwrootDirectory, UiTestHostConstants.AppSettingsProductionFileName);
            return true;
        }

        if (string.Equals(assetPath, UiTestHostConstants.AppSettingsDevelopmentCompressedFileName, StringComparison.Ordinal))
        {
            overridePath = Path.Combine(_appWwwrootDirectory, UiTestHostConstants.AppSettingsProductionCompressedFileName);
            return true;
        }
